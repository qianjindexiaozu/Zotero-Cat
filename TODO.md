...
- [x] 消息复制与复制成功反馈
- [x] 重试策略（未产生输出前的可恢复错误自动重试一次）
- [x] 历史会话管理（新建、清空、删除、原生下拉切换、每条目最多 8 条、按条目隔离）
- [x] 对话持久化（Zotero prefs JSON，保存活跃会话指针，带容量上限）
- [x] 等待响应动画（`.` `..` `...` 循环）
- [x] 响应等待耗时显示（本地从发送到首段可用输出的等待时间）
- [x] 固定高度聊天窗（90%）+ 自定义上下文折叠节省空间 + 自动滚动跟随
